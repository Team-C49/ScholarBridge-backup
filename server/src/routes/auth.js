...
router.post('/register/step1', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`SELECT 1 FROM users WHERE LOWER(email)=$1`, [norm]);
    ...
    await db.query(
      `INSERT INTO otps(id,user_email,otp_code,expires_at,created_at) VALUES($1,$2,$3,$4,now())`,
...
router.post('/register/step3', authMiddleware, requireRole('student'), async (req, res) => {
  try {
    ...
    if (bank_details && bank_details.account_number) {
      ...
      bankMasked = { account_masked: maskBank(bank_details.account_number), bank_name: bank_details.bank_name || null, ifsc: bank_details.ifsc || null };
    }

    const upsert = `
      INSERT INTO student_profiles(user_id, full_name, phone_number, date_of_birth, gender, address, profile_picture_url, kyc_doc_type, bank_details_cipher, bank_details_masked, created_at)
    ...
    await db.query(`INSERT INTO audit_logs(user_id, action, details) VALUES($1,'complete_profile',$2)`, [userId, JSON.stringify({ profile: rows[0] })]);
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const expiresAt = new Date(Date.now() + OTP_TTL_MIN * 60 * 1000);
    await db.query(`INSERT INTO otps(id,user_email,otp_code,expires_at,created_at) VALUES($1,$2,$3,$4,now())`, [uuidv4(), norm, otp, expiresAt]);
    await sendMail({ to: norm, subject: `${process.env.APP_NAME} - Password reset OTP`, text: `Your reset OTP: ${otp}` });
