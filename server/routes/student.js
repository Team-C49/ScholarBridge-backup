...
router.post('/applications', async (req, res) => {
  try {
    ...
    if (!academic_year || !total_amount_requested) return res.status(400).json({ error: 'academic_year and total_amount_requested required' });

    // snapshot current student profile (masked)
    const profQ = `SELECT full_name, phone_number, date_of_birth, gender, address, profile_picture_url, bank_details_masked FROM student_profiles WHERE user_id=$1`;
    const profRes = await db.query(profQ, [userId]);
    ...
    // Insert application
    const appId = uuidv4();
    const ins = `INSERT INTO applications(id, student_user_id, academic_year, current_course_name, school_college_name, total_amount_requested, application_snapshot)
    ...
    // Insert children (education_history, family_members, current_expenses)
...
// GET single application details (with children + approvals)
router.get('/applications/:id', async (req, res) => {
  try {
    const appId = req.params.id;
    const userId = req.user.id;
    // verify ownership
    const appQ = `SELECT * FROM applications WHERE id=$1 AND student_user_id=$2`;
    const a = await db.query(appQ, [appId, userId]);
    ...
    return res.json({ application: a.rows[0], education, family, expenses, docs, approvals });
  } catch (err) {
...
// DELETE application (allowed only if no approvals exist)
router.delete('/applications/:id', async (req, res) => {
  try {
    ...
    // check approvals
    const apr = await db.query(`SELECT COUNT(*) FROM application_approvals WHERE application_id=$1`, [appId]);
    if (Number(apr.rows[0].count) > 0) return res.status(400).json({ error: 'Cannot delete: approvals exist' });
...
// Student confirms receipt for a specific approval
router.post('/applications/:appId/confirm/:approvalId', async (req, res) => {
  try {
    ...
    // update approval: set student_confirmed_receipt = true
...
// Register a document (client uploads to storage, then calls this to save metadata)
router.post('/documents', async (req, res) => {
  try {
    ...
    const id = uuidv4();
    const sql = `INSERT INTO documents(id, owner_id, owner_type, doc_type, file_url, uploaded_by_user_id, created_at) VALUES($1,$2,$3,$4,$5,$6,now()) RETURNING *`;
