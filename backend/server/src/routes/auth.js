...
const { sendMail, sendOTPEmail } = require('../utils/mail');
...
router.post('/register/step1', async (req, res) => {
  try {
    ...
    await sendOTPEmail(norm, otp, OTP_TTL_MIN).catch((e) => console.warn('sendOTPEmail failed (non-fatal):', e.message));
...
router.post('/register/step3', authMiddleware, requireRole('student'), async (req, res) => {
  try {
    ...
    // Handle document URLs (from frontend uploads)
    let profile_picture_url = null;
    let kyc_document_url = null;

    // If documents were uploaded via the upload API, they'll have URLs
    if (profile_picture && profile_picture.url) {
      profile_picture_url = profile_picture.url;
    }

    if (kyc_document && kyc_document.url) {
      kyc_document_url = kyc_document.url;
    }
    ...
    // Update document ownership in documents table if documents were uploaded
    const documentsToUpdate = [];
    if (profile_picture && profile_picture.id) {
      documentsToUpdate.push({ id: profile_picture.id, type: 'profile_picture' });
    }
    if (kyc_document && kyc_document.id) {
      documentsToUpdate.push({ id: kyc_document.id, type: 'kyc_document' });
    }

    // Update document ownership to link them to the student profile
    for (const doc of documentsToUpdate) {
      await db.query(
        `UPDATE documents SET owner_id = $1, owner_type = 'student' WHERE id = $2`,
        [userId, doc.id]
      );
    }
    ...
    await db.query(`INSERT INTO audit_logs(user_id, action, details) VALUES($1,'complete_profile',$2)`, [
      ...
      JSON.stringify({ 
        profile: rows[0], 
        documents_linked: documentsToUpdate.length
      }),
    ]);

    return res.status(201).json({ 
      profile: rows[0],
      documentsLinked: documentsToUpdate.length
    });
  } catch (err) {
